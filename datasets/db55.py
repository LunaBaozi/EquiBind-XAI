...
class DB55(Dataset):
    ...
    def __init__(self, device='cuda:0',
                 ...
                 structures_path='data/DB55/structures/',
                 bsp_proteins=False,
                 bsp_ligands=False,
                 pocket_cutoff=8.0,
                 use_rec_atoms=False,
        ...
        # --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        if not os.path.exists('data/DB55/ready2process_u'):
            os.mkdir('data/DB55/ready2process_u')

        if not os.path.exists('data/DB55/bound_structures'):
            os.mkdir('data/DB55/bound_structures')

        for file in os.listdir(self.structures_path):
            filename = file[:4]
            # print(filename)
            if not os.path.exists(f'data/DB55/ready2process_u/{filename}'):
                os.mkdir(f'data/DB55/ready2process_u/{filename}')
                files2move = [file for file in os.listdir(structures_path) if filename in file and '_b' not in file]  # and filename != '1JPS'
                print(files2move)
            for file in files2move:
                if 'l' in file:
                    shutil.copyfile(f'data/DB55/structures/{file}', f'data/DB55/ready2process_u/{filename}/{file}')
                    os.rename(f'data/DB55/ready2process_u/{filename}/{file}', f'data/DB55/ready2process_u/{filename}/{filename}_ligand.pdb')
                elif 'r' in file:
                    shutil.copyfile(f'data/DB55/structures/{file}', f'data/DB55/ready2process_u/{filename}/{file}')
                    os.rename(f'data/DB55/ready2process_u/{filename}/{file}', f'data/DB55/ready2process_u/{filename}/{filename}_protein.pdb')
    

        for file in os.listdir(self.structures_path):
            filename = file[:4]    
            if not os.path.exists(f'data/DB55/bound_structures/{filename}'):
                os.mkdir(f'data/DB55/bound_structures/{filename}')
                files2move = [file for file in os.listdir(structures_path) if filename in file and '_b' in file]
            for file in files2move:
                if 'l' in file:
                    shutil.copyfile(f'data/DB55/structures/{file}', f'data/DB55/bound_structures/{filename}/{file}')
                    os.rename(f'data/DB55/bound_structures/{filename}/{file}', f'data/DB55/bound_structures/{filename}/{filename}_ligand.pdb')
                elif 'r' in file:
                    shutil.copyfile(f'data/DB55/structures/{file}', f'data/DB55/bound_structures/{filename}/{file}')
                    os.rename(f'data/DB55/bound_structures/{filename}/{file}', f'data/DB55/bound_structures/{filename}/{filename}_protein.pdb')

        tmp = list()
        with open('data/DB55/all_structures', 'w') as file:
            for name in os.listdir(self.structures_path):
                tmp.append(name[:4])
            structures = np.unique(np.array(tmp))
            for name in structures:
                file.writelines(name.lower() + '\n')
        ...
        if ((not os.path.exists(os.path.join(self.processed_dir, 'geometry_regularization.pt')) and self.geometry_regularization) 
            ...
            or not os.path.exists(os.path.join(self.processed_dir, 'rec_graphs.pt')) 
            or not os.path.exists(os.path.join(self.processed_dir, 'pocket_and_rec_coords.pt')) 
            or not os.path.exists(os.path.join(self.processed_dir, self.lig_graph_path)) 
            or (not os.path.exists(os.path.join(self.processed_dir, self.rec_subgraph_path)) and self.rec_subgraph) 
    ...
    def process(self):
        ...
        for name in tqdm(complex_names, desc='loading ligands'):
            if self.bsp_ligands:
                ...
                lig = read_molecule(os.path.join(self.db55_dir, name, f'{name}_ligand.sdf'), sanitize=True,
                                    remove_hs=self.remove_h)
                if lig == None:  # read mol2 file if sdf file cannot be sanitized
                    lig = read_molecule(os.path.join(self.db55_dir, name, f'{name}_ligand.mol2'), sanitize=True,
                                        remove_hs=self.remove_h)
        ...
        if self.bsp_proteins:
            ...
            rec_paths = [os.path.join(self.db55_dir, name, f'{name}_protein_processed.pdb') for name in
                         complex_names]
